...
describe("getPeerTools", () => {
  it("returns read_channel definition for no peers", () => {
    const { definitions } = getPeerTools([], "agent-1", selfUrl, noopLogger, testKey, mockDb);
    assert.equal(definitions.length, 1);
    assert.equal(definitions[0].name, "read_channel");
  });

  it("creates one tool definition per peer plus read_channel", () => {
    const peers = [
      { id: "alpha", url: "http://alpha:4201", description: "Research agent" },
      { id: "beta", url: "http://beta:4202", description: "Code agent" },
    ];
    const { definitions } = getPeerTools(peers, "agent-1", selfUrl, noopLogger, testKey, mockDb);
    assert.equal(definitions.length, 3);
    assert.equal(definitions[0].name, "message_alpha");
    assert.equal(definitions[1].name, "message_beta");
    assert.equal(definitions[2].name, "read_channel");
  });
  ...
  it("dispatch rejects unknown tool names", async () => {
    const { dispatch } = getPeerTools([], "agent-1", selfUrl, noopLogger, testKey, mockDb);
  ...
  it("dispatch returns error message when peer is unreachable", async () => {
    ...
    const { dispatch } = getPeerTools(peers, "agent-1", selfUrl, noopLogger, testKey, mockDb);
