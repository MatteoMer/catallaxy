...
const toolRegistry = createToolRegistry(config, logger, db);
...
const agent = createAgent(config.id, db, llm, logger, toolRegistry.peerDefinitions, toolRegistry.peerDispatch);
const app = createServer(agent, config, db);
